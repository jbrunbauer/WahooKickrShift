...
// jbr: Start 
// jbr: Declare Functions before first use
...
void DisplayText(String myDebug)
{
  // Create a sprite (bitmap) in RAM of defined width and height, graphics can then be drawn in the Sprite and rendered quickly onto the TFT screen
...
bool connectToServer(BLEAddress pAddress)
{
  String myDisplay = "Connect to " + String(pAddress.toString().c_str());
  DisplayText(myDisplay);

  BLEClient *pClient = BLEDevice::createClient();
  DisplayText("BLE Client created");
  ...
  DisplayText("Server connection established");

  // Gearing
  BLERemoteService *pRemoteService1 = pClient->getService(serviceUUID1);
  if (pRemoteService1 == nullptr)
  {
    myDisplay = "Failed to find service UUID1: " + String(serviceUUID1.toString().c_str());
    DisplayText(myDisplay);
    pClient->disconnect();
    return false;
  }
  DisplayText("Found Service UUID1");

  // Cycling Power Measurement
  BLERemoteService *pRemoteService2 = pClient->getService(serviceUUID2);
  if (pRemoteService2 == nullptr)
  {
    myDisplay = "Failed to find service UUID2: " + String(serviceUUID2.toString().c_str());
    DisplayText(myDisplay);
    pClient->disconnect();
    return false;
  }
  DisplayText("Found Service UUID2");

  // Weight
  BLERemoteService *pRemoteService3 = pClient->getService(serviceUUID3);
  if (pRemoteService3 == nullptr)
  {
    myDisplay = "Failed to find service UUID3: " + String(serviceUUID3.toString().c_str());
    DisplayText(myDisplay);
    pClient->disconnect();
    return false;
  }
  DisplayText("Found Service UUID3");
...
bool connectCharacteristic1(BLERemoteService *pRemoteService, BLERemoteCharacteristic *pRemoteCharacteristic)
{
  ...
  {
    DisplayText("Failed to find characteristic UUID1");
    return false;
  }
  DisplayText("Found Characteristic 1");
...
bool connectCharacteristic2(BLERemoteService *pRemoteService, BLERemoteCharacteristic *pRemoteCharacteristic)
{
  ...
  if (pRemoteCharacteristic == nullptr)
  {
    DisplayText("Failed to find characteristic UUID2");
    return false;
  }
  DisplayText("Found Characteristic 2");
...
bool connectCharacteristic3(BLERemoteService *pRemoteService, BLERemoteCharacteristic *pRemoteCharacteristic)
{
  ...
  if (pRemoteCharacteristic == nullptr)
  {
    DisplayText("Failed to find characteristic UUID3");
    return false;
  }
  DisplayText("Found Characteristic 3");
...
class MyAdvertisedDeviceCallbacks : public BLEAdvertisedDeviceCallbacks
{
  void onResult(BLEAdvertisedDevice advertisedDevice)
  {
    ...
    { // Check if the name of the advertiser matches
      DisplayText("Device found, start connecting ...");
...
void setup()
{
  ...
  // tft.init();
  tft.begin();
  // 0 = nicht drehen, 1 = 90°, 2 = 180°, 3 = 270°
  tft.setRotation(1); 
